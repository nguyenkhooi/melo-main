...
const DEFAULT_STATE = RootNavigator.router.getStateForAction(
  NavigationActions.init(),
  null
);

/**
 * @description Finds the current route.
 *
...
export const NavigationStoreModel = NavigationEvents.named("NavigationStore")
  ...
  .actions((self) => ({
    ...
    dispatch(action: NavigationAction, shouldPush = true) {
      ...
      self.state =
        RootNavigator.router.getStateForAction(action, previousNavState) ||
        self.state;
