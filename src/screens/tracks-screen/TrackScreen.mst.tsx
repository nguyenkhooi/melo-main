...
interface dTracksScr extends dSCR {}

function TracksScreen(props: dTracksScr) {
  const { navigation } = props;
  const [modal, setModal] = useState({ visible: false, item: {} });
  const {
    mediaStore: { g_fetchTracks, g_areTracksLoaded, g_tracks },
    configStore: { g_isFooterShown, g_toggleFooter },
  } = useStores();

  useEffect(
    function gettingStarted() {
      let unsubscribe = navigation.addListener("focus", () => {
        //* Show footer when screen is shown
        g_toggleFooter(true);
        g_fetchTracks();
      });
      return unsubscribe;
...
export default observer(TracksScreen);
